...
interface OfferModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (offer: Omit<Offer, 'id'> | Offer) => Promise<void>;
  offer: Offer | null;
}
...
const OfferModal: React.FC<OfferModalProps> = ({ isOpen, onClose, onSave, offer }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await onSave(offerToSave as Offer);
    } catch (e) {
      // Allow parent to handle toast, but stop submitting state here if fails
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div ref={modalRef} className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-md relative">
        ...
        <h2 id="modal-title" className="text-2xl font-bold mb-6 text-gray-800">{offer ? 'Edit Offer' : 'Add Offer'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <InputField name="title" label="Title" />
          
          <div>
            <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <textarea id="description" name="description" value={formData.description} onChange={handleChange} rows={3} placeholder="Enter a short description for the offer" className={`w-full p-3 border rounded-lg focus:ring-2 ${errors.description ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-[#2D7A79]'}`} aria-invalid={!!errors.description} />
            {errors.description && <p className="text-red-600 text-sm mt-1">{errors.description}</p>}
          </div>

          <InputField name="promoCode" label="Promo Code" />
